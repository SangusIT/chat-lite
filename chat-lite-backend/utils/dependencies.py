...
async def get_current_user(token: Annotated[str, Depends(oauth2_scheme)], client):
    ...
    try:
        ...
        raise credentials_exception
    user = await get_user(client, username=token_data.username)
    user = UserPrivate(**user)
    if user is None:
        raise credentials_exception
    return user
...
async def verify_key(key: str, client):
    user_key = await check_key(client, key)
    if not user_key:
